...
A continuación se presentan los gráficos que comparan las series históricas reales de casos registrados contra las proyecciones obtenidas mediante el modelo predictivo calibrado para cada una de las patologías bajo estudio, dispuestos en secuencia vertical:
...
\begin{itemize}
    \item \textbf{Efectividad del Modelo Predictivo:} La Suavización Exponencial Triple Holt-Winters con estacionalidad multiplicativa demostró ser una metodología matemática altamente efectiva para modelar brotes epidémicos estacionales no lineales, logrando capturar tanto la explosividad aguda del Dengue como los ciclos invernales de las patologías respiratorias (Bronquiolitis e Influenza).
    \item \textbf{Comportamiento de la Constante de Tendencia ($\beta \to 0$):} Al optimizar los parámetros de suavizamiento ($\alpha, \beta, \gamma$) en Excel Solver para minimizar la Desviación Absoluta Media ($MAD$), la constante de tendencia $\beta$ tendió sistemáticamente a cero ($\beta \approx 0$). Esto ocurre porque los brotes epidemiológicos estacionales no obedecen a una tendencia secular persistente de largo plazo, sino a un comportamiento puramente oscilatorio y cíclico anual regulado por el nivel base ($L_t$) y por factores estacionales multiplicativos muy marcados ($S_t$). Permitir una constante $\beta > 0$ generaría proyecciones con tendencias lineales espurias que amplificarían artificialmente los picos epidémicos o producirían valores negativos durante las fases interepidémicas; por ende, la anulación de $\beta$ por parte de Solver preserva la estabilidad del nivel y la periodicidad estacional real.
    \item \textbf{Recuperación Digital de Datos:} El uso de la herramienta WebPlotDigitizer permitió recuperar información cuantitativa de alta fidelidad a partir de curvas publicadas en reportes oficiales no estructurados, eliminando sesgos en el ingreso de datos históricos.
    \item \textbf{Desacoplamiento Operativo:} La Tabla de Uso demostró ser una interfaz funcional efectiva para desacoplar y comunicar secuencialmente un modelo dinámico de series temporales con un modelo estático y discreto de optimización semanal.
