...
\subsection{Formulación Matemática de Holt-Winters Multiplicativo}
El modelo de Suavización Exponencial Triple descompone la serie temporal en tres componentes mediante las siguientes ecuaciones de estado actualizadas semanalmente:
\begin{equation}
...
El sistema predictivo opera bajo los siguientes índices, horizontes y constantes:
\begin{itemize}
    \item $t$: Índice temporal de la semana epidemiológica específica en la serie ($t = 1, 2, \dots, n$).
    \item $i$: Patología bajo análisis (donde $i=1$ es Dengue, $i=2$ es Bronquiolitis y $i=3$ es Influenza).
    \item $s$: Longitud del ciclo estacional anual ($s = 52$ semanas).
    \item $m$: Horizonte temporal de pronóstico futuro.
    \item Constantes de suavizamiento optimizadas en el intervalo $[0,1]$: de nivel ($\alpha$), de tendencia ($\beta$) y de estacionalidad ($\gamma$).
\end{itemize}

\subsubsection{Criterio de Selección de Ciclos Históricos y Ventanas Temporales}
La delimitación de las ventanas de datos históricos para alimentar el modelo responde a las características biológicas y de transmisión de cada patología:
\begin{itemize}
...
\subsubsection{Origen de Datos y Digitalización de Gráficos}
...
Debido a que múltiples reportes históricos solo presentaban las curvas epidemiológicas en formato gráfico no editable (PDFs escaneados), se utilizó la herramienta digitalizadora \textbf{WebPlotDigitizer} para realizar una calibración de ejes cartesianos y extraer con precisión milimétrica las coordenadas numéricas de casos semanales reales.
...
Para hallar la combinación óptima de las constantes de suavizamiento ($\alpha, \beta, \gamma$), se empleó el Solver de Microsoft Excel, configurando como celda objetivo la minimización de la Suma de Errores Cuadráticos ($SEC$) sobre los datos históricos. 
